...
@Slf4j // 로그라이브러리를 사용하기 위해 @Slf4j를 추가
@Service
public class TodoService {
	...
	public List<TodoEntity> update(final TodoEntity entity) {
		...
		// 2.3.2 Retrueve Todo에서 만든 메서드를 이용해 유저의 모든 Todo 리스트를 리턴한다.
	...
	public List<TodoEntity> delete(final TodoEntity entity) {
		...
		try {
			...
			// (4) 컨트롤러로 exception을 날린다. DB 내부 로직을 캡슐화 하기 위해 e를 리턴하지 않고 새 exception 오브젝트를
			// 리턴한다.
